...
class Robustness(object):
    """ For each tracklet.
    Computes and stores the roubstness, which is based on the frame length when the first failure happens
    """
    ...
    def value(self):
        ...
        for thres in self.Xaxis:
            track_len = -1
            for i, o in enumerate(self.overlaps):
                if o < thres:
                    track_len = i + 1
                    break
            if track_len == -1:
                track_len = self.count
            robust.append(track_len / self.count)
        return np.array(robust)

    @property
    def average(self):
        if len(self.overlaps) == 0:
            return 0
        return self.compute_area(self.value) * 100 / self.max_overlap
