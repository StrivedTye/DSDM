...
criterion_similarity = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([2.0]), reduction='none') #nn.MSELoss()
...
mu_sim, mu_reg, mu_seg, mu_reg_proposal= 1, 1, 0.5, 1
num_seeds = 64
...
for epoch in range(opt.nepoch):
    ...
    for i, data in enumerate(tqdm(train_dataloader, 0, ncols=120)):
        ...
        search_box_all, delta_p_all, score_all, t_score, search_score, \
          _, pred_proposal, pred_centers = netR(search_PC, t_PC, search_origin_state, t_origin_state)

        # generate label of proposal
        label_proposal_loc = search_gt_state.repeat(1, pred_proposal.shape[1], 1)
        label_proposal_loc = label_proposal_loc[:, :, [0, 1, 2, 6]] #[B, N, 4]
        ...
        loss_proposal_reg = criterion_proposal_reg(pred_proposal,
                                                   label_proposal_loc,
                                                   label_search_semantic.squeeze(-1))

        loss_proposal_reg = loss_proposal_reg.mean()
